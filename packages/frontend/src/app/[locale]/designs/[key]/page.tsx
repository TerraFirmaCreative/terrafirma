...
const ViewCustomMats = ({ params }: { params: { locale: string, key: string } }) => {
  const [selected, setSelected] = useState(1)
  const [promptOpen, setPromptOpen] = useState(false)
  const { products, vary, inProgress, refreshProducts } = useContext(CreationContext)
  const [fetching, setFetching] = useState<boolean>(true)
  const [filteredProducts, setFilteredProducts] = useState<ProductWithImagineData[]>([])
  ...
  useEffect(() => {
    setFilteredProducts(
      products.filter((p) => {
        console.log(trimPrompt(p!.imagineData!.imaginePrompt), params.key)
        return encodeURIComponent(trimPrompt(p!.imagineData!.imaginePrompt)).startsWith(params.key)
      }
      )
    )
  }, [products, params.key])
  ...
  useEffect(() => {
    ...
    setFilteredProducts(
      products.filter((p) =>
        trimPrompt(p!.imagineData!.imaginePrompt).includes(trimPrompt(params.key))
      )
    )
  }, [])
  ...
  return (
    <>
      <div className="flex lg:flex-row mt-20 flex-col justify-start min-h-[calc(100vh-5rem)] h-full">
        <div className="lg:w-1/2 px-20 pt-2 align-center h-full flex flex-col justify-center items-center max-w-full">
          {
            filteredProducts.length > 4
              ? <DesignGrid products={filteredProducts} setSelected={setSelected} />
              : <GeneratedImageCarousel products={filteredProducts} setSelected={setSelected} />
          }
        </div>
